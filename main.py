...
def preprocess_image(
    ...
    binary_output: str = "binary_omr.png",
) -> np.ndarray:
...
def detect_filled_bubbles(
    img_thresh: np.ndarray, sections: List[Tuple], img_shape: Tuple[int, int]
) -> Dict[str, List[Tuple[Union[int, None], Union[int, str]]]]:
...
def draw_detected_areas(
    ...
    for section in sections:
        section_name, x1_perc, x2_perc, y1_perc, y2_perc, options, num_questions, _ = (
            section
        )
        x1 = int(x1_perc / 100 * width)
        x2 = int(x2_perc / 100 * width)
        ...
        for q in range(num_questions):
            for o, option in enumerate(options):
                ...
                cy2 = int(y1 + (q + 1) * row_height)
                cv2.rectangle(
                    image, (cx1, cy1), (cx2, cy2), (255, 0, 0), 1
                )  # Red box for each bubble

                # If the bubble is filled, draw a green box
                if (q + 1, option) in filled_bubbles.get(section_name, []):
                    cv2.rectangle(
                        image, (cx1, cy1), (cx2, cy2), (0, 255, 0), 2
                    )  # Green box for filled bubble
...
def scan_omr(
    image_path, output_path: str = "scanned_omr.png", show: bool = False
) -> Dict[str, List[Tuple[Union[int, None], Union[int, str]]]]:
    ...
    filled_bubbles = detect_filled_bubbles(
        img_thresh, sections, (img_thresh.shape[0], img_thresh.shape[1])
    )

    # Draw the detected bubbles on the original image
    result_img = draw_detected_areas(
        original_img.copy(),
        filled_bubbles,
        sections,
...
def merge_mcq_sections(
    filled_bubbles: Dict[str, List[Tuple[Union[int, None], Union[int, str]]]],
) -> List[Union[str, int]]:
